* table 栏位调整，
* 不属于你的范围
* auto compelete：组件修改，换为option的动作 --＞　必须选择，而不是输入，有做优化
* 绘制加减号，和UI不符合；将自己拿掉，就不会重复
* 不确定的事情要及时举手
* 2 s的时候，有重复工作的步骤
* 重构的时候，保证功能正常运行

### 新增数据源，在releasemgmt上面的操作

```json
jdbcUrl is required with driverClassName.",
```

报错以上信息，标识没有找到jdbcurl

在valut上面:	

1. 新增对应的username、jdbc-url、password的值

在realsemgmt上面:	

1. resourcepool增加对应的valut的key

在argocd上:

1. 等待resourcepool refresh成功
2. restart 引用到这个valut的api
...
![1744362067718](image/快速笔记/1744362067718.png)

### 番茄工作法

1. 任务拆分成25分钟可以完成的
2. 25分钟后休息5分钟，喝水或者闭目养神，切忌刷手机，碎片化时间浪费更精力
3. 2h休息15-30min
